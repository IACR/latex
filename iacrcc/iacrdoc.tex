...
\title[running  = {The iacrcc class},
       subtitle = {iacrcc LaTeX Class Documentation (v0.32)}
...
\subsection{Title}
A title is added using the \cmd{title} macro, it has a number of (optional) options:
\begin{center}
...
There are a number of arguments to \cmd{addaffiliation}:
...
\texttt{textabstract} environment, and must be free of macros or
mathematical content. It will be used for indexing and production of
...
You may use either \BibTeX\ or \texttt{biblatex}. If you use \BibTeX,
then the \texttt{iacrcc} class will load the
\verb+\bibliographystyle{alphaurl}+ style.  You may not change
this. If you use \texttt{biblatex}, then this is done using
