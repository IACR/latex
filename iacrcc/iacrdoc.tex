...
\title[running  = {The iacrcc class},
       subtitle = {iacrcc LaTeX Class Documentation (v0.58)}
...
diacriticals like \verb+F\"ur Elise+. You may also use (inline)
mathematics in the \texttt{textabstract} environment as well as
(for example) the itemize environment. Use-defined macros are
\emph{not} allowed. We do not have a complete list of allowed
\LaTeX (which can be succesfully converted to HTML) but when
in doubt try and compile the final version at
\url{https://publish.iacr.org}.
