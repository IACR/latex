...
\title[running  = {The iacrcc class},
       subtitle = {iacrcc LaTeX Class Documentation (v0.78)}
...
\item Avoid using too many packages. Many authors are lazy and just
  copy what they used in the past. The compile system used for
  IACR CiC includes about 2000 packages, but not everything. Some packages
  are explicitly forbidden, and these are listed at
  \url{https://publish.iacr.org/forbidden}.
\item Don't try to change the \texttt{hyperref} options, the bibliography
  style, the page style, or the page numbering. \texttt{iacrcc.cls} already
  loads the \texttt{hyperref} package.
...
to provide a supported license using the \cmd{license} macro
in the preamble of the document.  In all other modes this information
...
\verb+\bibliographystyle{iacrcc}+ style.  You may not change
...
\section{\texttt{hyperref}, \texttt{cleveref}, and package load order}\label{sec:loadorder}

The \texttt{iacrcc} document class loads \texttt{hyperref} after all
other packages have been loaded (including those loaded by the user in
the preamble). It's possible that you may wish to load some packages after
\texttt{hyperref}. The way to do this is
to create a file \texttt{after-hyperref.sty} in the same directory as
your main file and load things from there. Note that we do {\em not}
recommend use of the \texttt{cleveref} package because it is no longer
maintained and has some bugs. We recommend using
the \texttt{zref-clever} package instead if you desire the features of
\texttt{cleveref}.
