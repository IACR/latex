...
\usepackage[cityrequired,maketitle=acmconf]{metacapture}
\def\pkgversion{\METAC@fileversion}
\date{\METAC@filedate}
\makeatother
...
\subsection{Title}
A title is added using the \cmd{title} macro, which has two optional arguments:

\newcommand{\argrow}[2]{\texttt{#1} & #2\\}
\newenvironment{arglist}{%
  \renewcommand{\arraystretch}{1.2}\begin{tabular}{@{}lp{0.7\linewidth}}%
  }{%
  \end{tabular}
  }
\begin{arglist}
...
\argrow{roles}{A comma-separated list of roles that correspond to the values
   in Table~\ref{rolevalues}.}
\end{arglist}
\end{center}
...
\begin{tabular}{ll}
\toprule
Crossref roles & \CREDIT\ roles \\
\midrule
\texttt{author} & \texttt{conceptualization} \\
\texttt{editor} & \texttt{data-curation} \\
\texttt{chair} & \texttt{formal-analysis} \\
\texttt{reviewer} & \texttt{funding-acquisition} \\
\texttt{review-assistant} & \texttt{investigation} \\
\texttt{stats-reviewer} & \texttt{methodology} \\
\texttt{reviewer-external} & \texttt{project-administration} \\
\texttt{reader} & \texttt{resources} \\
\texttt{translator} & \texttt{software} \\
\texttt{corresponding-author} & \texttt{supervision} \\
\texttt{other} & \texttt{validation} \\
& \texttt{visualization} \\
& \texttt{writing-original-draft} \\
& \texttt{writing-review-editing} \\
\bottomrule
\end{tabular}
\caption{Possible value for the \texttt{roles} attribute of \cmd{addauthor}.
These come from one of two vocabularies that are both supported by Crossref.
See Appendix~\ref{roles} for more information about these, and see the discussion of
contributor roles in Section~\ref{missing} for information about how roles might be marked
up in a document.}
\label{rolevalues}
\end{table}

As we mentioned in section~\ref{maketitle}, the display of these
elements by a document class may be customized in
...
but the
\href{https://info.orcid.org/documentation/integration-guide/orcid-id-display-guidelines/#h-unauthenticated-orcid-ids-0}{unauthenticated
...
\texttt{Crossref} currently has a required element for
surname in their 5.5 schema, which is why we include this. They are
in the process of revising their requirements in forthcoming schemas, but
surname will remain here for convenience of sorting.
...
\addauthor[orcid    = {0000-0000-0000-0000},
           ...
           roles    = {author,corresponding-author,software}
          ]{Alice Accomplished}
\end{Verbatim}

The \cmd{thanks} macro is disabled
inside \cmd{addauthor}, so use the \verb+footnote+ option
on \cmd{addauthor} instead. In fact, it
generates an error if an author attempts to use any
non-accent macros inside the primary argument to \cmd{addauthor}.
...
metadata for indexing. If an author wishes to include such visible
annotations, they can use the \texttt{footnote} option on
...
\item[Contributor roles] \label{rolestodo} Author roles were
added to this package in version 0.9.2. The schema is the same
as what Crossref supports in version~5.5 of their schema. These expand
upon the \CREDIT\ roles which are used in JATS.\footnote{See \url{https://casrai.org/credit/jats}}
We do not yet support varying levels of a role (neither does Crossref).

There are several things that remain to be determined, like whether to
display a non-author contributor in the front matter produced
by \cmd{maketitle}.  We only provide roles as part of \cmd{addauthor},
but a document class might also allow contributors to
be noted without listing them in \cmd{addauthor}.  This is what
the \cmd{contrib} macro from \pkg{amsart.cls} does.  We regard this as
a decision for the document class. It's possible that a document class
could relegate contributors added by \cmd{addauthor} but with
non-author roles to simply foot notes or acknowledgements rather than
listing them in \cmd{maketitle}.  Our implementations
of \cmd{maketitle} treat all contributors the same and display them as
authors.

There is also the question of how contributor roles should be displayed
in the document. There are several common ways that this is done, including
\begin{itemize}
\item An author-centric back matter section where roles are listed for each author.
We implement this with the \cmd{ShowRoles} macro.
\item Footnotes placed on authors to indicate their roles,
\item A role-centric back matter section in which the roles are listed, and for each
role, a list of contributors with that role is provided.
\item The \pkg{credits} \LaTeX\
package\footnote{See \url{https://github.com/Pseudomanifold/latex-credits}}
provides several ways to display the \CREDIT\ roles, including as a
matrix from role to contributor (or contributor to
role).
\end{itemize}
We regard these layout enhancements as something that is properly left
to a document style, since they are about \textit{display} rather than
capture of metadata.

It's noteworthy that multiple articles have pointed out weaknesses in
the \CREDIT\ taxonomy~(e.g., \cite{Godskesen19052026}), and it may be
premature to standardize on this (or the Crossref extension).

\item[AI Usage] The usage of AI by authors is becoming ubiquitous, but at present
the trend seems to be to disallow listing AI as an author. Many journals and conferences
are starting to place disclosure requirements on authors, but we believe that it is
too early to standardize such a disclosure as structured data.
...
\ShowRoles %[layout=list]
\appendix
\section{Appendix: Author roles}\label{roles}
The \texttt{roles} attribute for \cmd{addauthor} contains designations
for entries in two standard vocabularies, namely \CREDIT\ and Crossref
(the possible values are listed in Figure~\ref{rolevalues}).  Both
vocabularies are supported in
the \href{https://data.crossref.org/reports/help/schema_doc/5.5.0/index.html}{Crossref
schema version~5.5}.  The \CREDIT\ roles have fairly precise
definitions, and
we \href{https://doi.org/10.5281/zenodo.18421448}{refer to those
directly}.

As far as we know,
Crossref has never published official defintions for their roles. In order
to clarify these, we provide our interpretation of their intent below.
\begin{description}

\item[\texttt{author}] (based on \href{https://www.icmje.org/recommendations/browse/roles-and-responsibilities/defining-the-role-of-authors-and-contributors.html}{ICMJE's four criteria})
An individual who made substantial intellectual contributions to the design, execution, or interpretation of the work; drafted or revised it; approved the final version; and is accountable for its integrity.
\item[\texttt{corresponding-author}] This has an ambiguous definition. ICMJE calls it
``the individual "who takes primary responsibility for communication with the journal
during the manuscript submission, peer review, and publication process.'' The JATS definition
is that this is an author who is designated as a person to receive correspondence for
the article.
\item[\texttt{editor}] An individual responsible for selecting, evaluating, revising, and organizing the content for publication, ensuring editorial independence and scientific integrity.
\item[\texttt{translator}] A linguist or scholar responsible for translating the text from its original language into another
while preserving meaning, register, and technical accuracy, including the translator's own scholarly judgement on terminology, idiom, and disciplinary convention. (see \href{https://casrai.org/dictionary/term/translator-contribution}{CASRAI})
\item[\texttt{chair}] (from IEEE) The designated leader or coordinator of a conference session, symposium, or working group responsible for oversight of the presented proceedings.
\item[\texttt{reviewer}] An independent expert in the same field who evaluates the manuscript's methodology, originality,
validity, and clarity to assist the editor in decision-making. This is probably only relevant if the article is itself
a peer review.
\item[\texttt{stats-reviewer}] (from \href{https://www.icmje.org/recommendations/browse/roles-and-responsibilities/responsibilities-in-the-submission-and-peer-peview-process.html}{ICMJE Section II.C})
A specialized methodology expert explicitly tasked with evaluating the data handling, statistical power, mathematical models, and quantitative logic of the paper.
\item[\texttt{reviewer-external}] An independent subject-matter expert brought in from outside to adjudicate highly specialized content.
\item[\texttt{review-assistant}] (from COPE Co-Reviewing) A junior scholar or co-reviewer who assists a primary invited reviewer in reading the text and drafting feedback.
\item[\texttt{reader}] An individual who participates in the creation of the work by proof-reading or performing an independent evaluation.
\end{description}
