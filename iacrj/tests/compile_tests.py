...
def run_engine(eng, filelist, tmpdirpath):
  ...
  shutil.copy('../../metacapture/metacapture.sty', tmpdirpath)
...
def test11_test():
  ...
  for option in ['-pdflua', '-pdf']:
    with tempfile.TemporaryDirectory() as tmpdirpath:
      ...
      assert subject[0] == 'Dirac delta function'
      assert subject[1] == 'unit impulse'
...
def test16_test():
  ...
  for option in ['-pdflua', '-pdf']:
    ...
    with tempfile.TemporaryDirectory() as tmpdirpath:
      ...
      meta = meta_parse.parse_meta(res['meta'])
      assert meta['title'] == 'An example that is not anonymous'
      assert meta['subtitle'] == 'A LaTeX Templåte'
...
def test23_test():
  ...
  for option in ['-pdf','-pdflua']:
    ...
    with tempfile.TemporaryDirectory() as tmpdirpath:
      ...
      del line[1]
      del line[0]
      assert line[0] == r"title: On the possibility of basing Cryptography on the assumption that $P \neq  NP$"
      assert line[1] == r"title: Black-Box Concurrent Zero-Knowledge Requires $\tilde  \Omega (\log n)$ Rounds"
      assert line[2] == r"title: Parallel scalar multiplication on general elliptic curves over $\mathbb  {F}_p$ hedged against Non-Differential Side-Channel Attacks"
      ...
      assert line[7] == r"title: Isomorphism Classes of Hyperelliptic Curves of Genus 2 over $\mathbb  {F}_{2^n}$"
      ...
      assert line[13] == r"title: Classification of genus 2 curves over $\mathbb  {F}_{2^n}$ and optimization of their arithmetic"
      ...
      assert line[19] == r"title: Applications of $\mathcal  {M}$ultivariate $\mathcal  {Q}$uadratic Public Key Systems"
      ...
      assert line[25] == r"title: A Metric on the Set of Elliptic Curves over ${\mathbf  F}_p$."
      ...
      assert line[37] == r"title: Secure and {\sl  Practical} Identity-Based Encryption"
      ...
      assert line[47] == r"title: There exist Boolean functions on $n$ (odd) variables having nonlinearity $> 2^{n-1} - 2^{\frac  {n-1}{2}}$ if and only if $n > 7$"
      ...
      assert line[59] == r"title: Edon--${\cal  R}(256,384,512)$ -- an Efficient Implementation of Edon--${\cal  R}$ Family of Cryptographic Hash Functions"
      ...
      assert line[72] == r"title: A Comparison Between Hardware Accelerators for the Modified Tate Pairing over $\mathbb  {F}_{2^m}$ and $\mathbb  {F}_{3^m}$"
      ...
      assert line[79] == r"title: Polynomials for Ate Pairing and $\mathbf  {Ate}_{i}$ Pairing"
      ...
      assert line[97] == r"title: On the Randomness and Regularity of Reduced EDON-$\mathcal  {R}$ Compression Function"
      assert line[98] == r"title: Differential Path for SHA-1 with complexity $O(2^{52})$"
      assert line[99] == r"title: Algebraic Attacks specialized to \(\mathbb  {F}_2\) (Diplomarbeit)"
      assert line[100] == r"title: FPGA Implementations of SHA-3 Candidates:CubeHash, Grøstl, L{\sc  ane}, Shabal and Spectral Hash"
      ...
      assert line[103] == r"title: A short Note on Discrete Log Problem in $\mathbb  {F_p}$"
      ...
      assert line[108] == r"title: Improving the Berlekamp algorithm for binomials \boldmath  $x^{n} - a$"
      ...
      assert line[115] == r"title: Reducing Elliptic Curve Logarithm to Logarithm in a Finite Field $\mathbb  {F}_q$ for Some Orders"
      ...
      assert line[124] == r"title: On second-order nonlinearities of some $\mathcal  {D}_0$ type bent functions"
      ...
      assert line[137] == r"title: Number formula and degree level of ergodic polynomial functions over $\mathbb  {Z}$/$2^{n}\mathbb  {Z}$ and generalized result of linear equation on ergodic power-series T-Function"
      assert line[138] == r"title: Linear Approximations of Addition Modulo $2^n$-1"
      assert line[139] == r"title: RNS arithmetic in ${\mathbb  F}_{p^k}$ and application to fast pairing computation"
      ...
      assert line[151] == r"title: Cryptanalysis of Chen \textit  {et al.}'s RFID Access Control Protocol"
      assert line[152] == r"title: On the Number of Carries Occuring in an Addition $\mod  2^k-1$"
      ...
      assert line[158] == r"title: Cryptanalysis of Cho \textit  {et al.}'s Protocol, A Hash-Based Mutual Authentication Protocol for RFID Systems"
      ...
      assert line[164] == r"title: On a generalized combinatorial conjecture involving addition $\mod  2^k - 1$"
      assert line[165] == r"title: Cryptanalysis of improved Yeh \textit  {et al. }'s authentication Protocol: An EPC Class-1 Generation-2 standard compliant protocol"
      ...
      assert line[168] == r"title: Randomness Extraction in finite fields $\mathbb  {F}_{p^{n}}$"
      ...
      assert line[203] == r"title: New Impossible Differential Attack on $\text  {SAFER}_{+}$ and $\text  {SAFER}_{++}$"
      ...
      assert line[215] == r"title: Key Recovery Attacks on 3-round Even-Mansour, 8-step LED-128, and Full $\mbox  {AES}^{2}$"
      assert line[216] == r"title: Efficient Cryptosystems From $2^k$-th Power Residue Symbols"
      assert line[217] == r"title: Fast Exhaustive Search for Quadratic Systems in $\mathbb  {F}_2$ on FPGAs --- Extended Version"
      ...
      assert line[235] == r"title: Breaking `128-bit Secure' Supersingular Binary Curves (or how to solve discrete logarithms in ${\mathbb  F}_{2^{4 \cdot 1223}}$ and ${\mathbb  F}_{2^{12 \cdot 367}}$)"
      ...
      assert line[237] == r"title: Remarks on the Pocklington and Padró-Sáez Cube Root Algorithm in $\mathbb  F_q$"
      ...
      assert line[252] == r"title: New Class of Multivariate Public Key Cryptosystem, K(XI)RSE(2)PKC, Constructed based on Reed-Solomon Code Along with K(X)RSE(2)PKC over $\mathbb  {F}_2$"
      ...
      assert line[255] == r"title: Faster ECC over $\mathbb  {F}_{2^{521}-1}$"
      ...
      assert line[282] == r"title: Related-Key Rectangle Attack on Round-reduced \textit  {Khudra} Block Cipher"
      ...
      assert line[298] == r"title: Construction of $n$-variable ($n\equiv 2 \bmod  4$) balanced Boolean functions with maximum absolute value in autocorrelation spectra $< 2^{\frac  n2}$"
      ...
      assert line[310] == r"title: May-Ozerov Algorithm for Nearest-Neighbor Problem over $\mathbb  {F}_{q}$ and Its Application to Information Set Decoding"
      assert line[311] == r"title: \(\mu \)Kummer: efficient hyperelliptic signatures and key exchange on microcontrollers"
      assert line[312] == r"title: Malleability of the blockchain’s entropy"
      assert line[313] == r"title: Automatic Search for the Best Trails in ARX: Application to Block Cipher \textsc  {Speck}"
      ...
      assert line[319] == r"title: The Lightest 4x4 MDS Matrices over $GL(4,\mathbb  {F}_2)$"
      assert line[320] == r"title: Software Benchmarking of the 2$^{\text  {nd}}$ round CAESAR Candidates"
      ...
      assert line[333] == r"title: A Certain Family of Subgroups of $\mathbb  Z_n^\star $ Is Weakly Pseudo-Free under the General Integer Factoring Intractability Assumption"
      ...
      assert line[343] == r"title: Efficient hash maps to \mathbb  {G}_2 on BLS curves"
      ...
      assert line[362] == r"title: Faster multiplication in $\mathbb  {Z}_{2^m}[x]$ on Cortex-M4 to speed up NIST PQC candidates"
      ...
      assert line[375] == r"title: Efficient Range ORAM with $\mathbb  {O}(\log ^{2}{N})$ Locality"
      ...
      assert line[388] == r"title: How to leverage hardness of constant-degree expanding polynomials over $\mathbb  {R}$ to build iO"
      ...
      assert line[390] == r"title: Upper Bound on $\lambda _1(\Lambda ^{\bot }(\mathbf  A))$"
      ...
      assert line[393] == r"title: New point compression method for elliptic $\mathbb  {F}_{\!q^2}$-curves of $j$-invariant $0$"
      ...
      assert line[397] == r"title: Simplifying Constructions and Assumptions for $i\mathcal  {O}$"
      ...
      assert line[410] == r"title: Solving $x^{2^k+1}+x+a=0$ in $\mathbb  {F}_{2^n}$ with $\gcd (n,k)=1$"
      ...
      assert line[416] == r"title: Solutions of $x^{q^k}+\cdots  +x^{q}+x=a$ in $GF(2^n)$"
      ...
      assert line[419] == r"title: Scrutinizing the Tower Field Implementation of the $\mathbb  {F}_{2^8}$ Inverter -- with Applications to AES, Camellia, and SM4"
      assert line[420] == r"title: On cryptographic parameters of permutation polynomials of the form $x^rh(x^{(q-1)/d})$"
      assert line[421] == r"title: Don't forget your roots: constant-time root finding over $\mathbb  {F}_{2^m}$"
      assert line[422] == r"title: Can we Beat the Square Root Bound for ECDLP over $\mathbb  {F}_{p^2}$ via Representations?"
      ...
      assert line[424] == r"title: Efficient Information-Theoretic Secure Multiparty Computation over $\mathbb  {Z}/p^k \mathbb  {Z}$ via Galois Rings"
      ...
      assert line[431] == r"title: CCA-Secure Leakage-Resilient Identity-Based Key-Encapsulation from Simple (not $\mathtt  {q}$-type) Assumptions"
      ...
      assert line[442] == r"title: Breaking the $O(\sqrt  n)$-Bit Barrier: Byzantine Agreement with Polylog Bits Per Party"
      ...
      assert line[469] == r"title: Not enough LESS: An improved algorithm for solving Code Equivalence Problems over $\mathbb  {F}_q$"
      ...
      assert line[478] == r"title: A Family of Nonlinear MDS Diffusion Layers over $\mathbb  {F}_{2^{4n}}$"
      ...
      assert line[483] == r"title: Limits of Polynomial Packings for $\mathbb  {Z}_{p^k}$ and $\mathbb  {F}_{p^k}$"
      ...
      assert line[496] == r"title: Parallel Repetition of $(k_1,\dots  ,k_{\mu })$-Special-Sound Multi-Round Interactive Proofs"
      ...
      assert line[499] == r"title: MHz2k: MPC from HE over $\mathbb  {Z}_{2^k}$ with New Packing, Simpler Reshare, and Better ZKP"
      assert line[500] == r"title: Log-$\mathcal  {S}$-unit lattices using Explicit Stickelberger Generators to solve Approx Ideal-SVP"
      ...
      assert line[505] == r"title: \(\chi \)perbp: a Cloud-based Lightweight Mutual Authentication Protocol"
      assert line[506] == r"title: A State-Separating Proof for Yao’s Garbling Scheme"
      assert line[507] == r"title: Interactive Error Correcting Codes Over Binary Erasure Channels Resilient to $>\frac  12$ Adversarial Corruption"
      assert line[508] == r"title: Practical Garbled RAM: GRAM with $O(\log ^2 n)$ Overhead"
      assert line[509] == r"title: Just how hard are rotations of $\mathbb  {Z}^n$? Algorithms and cryptography with the simplest lattice"
      assert line[510] == r"title: Generating cryptographically-strong random lattice bases and recognizing rotations of $\mathbb  {Z}^n$"
      assert line[511] == r"title: Improved Security Bound of \textsf  {(E/D)WCDM}"
      ...
      assert line[514] == r"title: A Simple Deterministic Algorithm for Systems of Quadratic Polynomials over $\mathbb  {F}_2$"
      ...
      assert line[518] == r"title: Invertible Quadratic Non-Linear Layers for MPC-/FHE-/ZK-Friendly Schemes over $\mathbb  F_p^n$"
      ...
      assert line[521] == r"title: Indifferentiable hashing to ordinary elliptic $\mathbb  {F}_{\!q}$-curves of $j=0$ with the cost of one exponentiation in $\mathbb  {F}_{\!q}$"
      assert line[522] == r"title: A Compressed $\Sigma $-Protocol Theory for Lattices"
      assert line[523] == r"title: An $O(\log ^2 p)$ Approach to Point-Counting on Elliptic Curves From a Prominent Family Over the Prime Field $\mathbb  {F}_p$"
      ...
      assert line[530] == r"title: Delegating Supersingular Isogenies over $\mathbb  {F}_{p^2}$ with Cryptographic Applications"
      assert line[531] == r"title: LogStack: Stacked Garbling with $O(b \log b)$ Computation"
      assert line[532] == r"title: On the Possibility of Basing Cryptography on $EXP\neq  BPP$"
      ...
      assert line[539] == r"title: Faster indifferentiable hashing to elliptic $\mathbb  {F}_{\!q^2}$-curves"
      ...
      assert line[542] == r"title: Appenzeller to Brie: Efficient Zero-Knowledge Proofs for Mixed-Mode Arithmetic and $\mathbb  {Z}_{2^k}$"
      assert line[543] == r"title: A Note on ``Reduction Modulo $2^{448}-2^{224}-1$''"
      assert line[544] == r"title: SNARGs for $\mathcal  {P}$ from LWE"
      ...
      assert line[557] == r"title: Explicit infinite families of bent functions outside $\mathcal  {MM}^\#$"
      assert line[558] == r"title: Threshold Linearly Homomorphic Encryption on $\mathbf  {Z}/2^k\mathbf  {Z}$"
      assert line[559] == r"title: The Scholz conjecture on addition chain is true for $v(n)= 4$"
      assert line[560] == r"title: On the Performance Gap of a Generic C Optimized Assembler and Wide Vector Extensions for Masked Software with an Ascon-{\it  {p}} test case"
      assert line[561] == r"title: Second-Order Low-Randomness $d+1$ Hardware Sharing of the AES"
      assert line[562] == r"title: Bounded Surjective Quadratic Functions over $\mathbb  F_p^n$ for MPC-/ZK-/FHE-Friendly Symmetric Primitives"
      ...
      assert line[569] == r"title: Applications of the indirect sum in the design of several special classes of bent functions outside the completed $\mathcal  {MM}$ class"
      assert line[570] == r"title: Polynomial-Time Cryptanalysis of the Subspace Flooding Assumption for Post-Quantum $i\mathcal  {O}$"
      ...
      assert line[575] == r"title: Guaranteed Output in $O(\sqrt  {n})$ Rounds for Round-Robin Sampling Protocols"
      ...
      assert line[580] == r"title: Fast Subgroup Membership Testings for $\mathbb  {G}_1$, $\mathbb  {G}_2$ and $\mathbb  {G}_T$ on Pairing-friendly Curves"
      ...
      assert line[587] == r"title: Two new classes of permutation trinomials over $\mathbb  {F}_{q^3}$ with odd characteristic"
      ...
      assert line[595] == r"title: The $c-$differential uniformity and boomerang uniformity of three classes of permutation polynomials over $\mathbb  {F}_{2^n}$"
      assert line[596] == r"title: Proof of Mirror Theory for a Wide Range of $\xi _{\max }$"
      assert line[597] == r"title: Tight Multi-User Security Bound of $\textsf  {DbHtS}$"
      ...
      assert line[603] == r"title: More Efficient Dishonest Majority Secure Computation over $\mathbb  {Z}_{2^k}$ via Galois Rings"
      assert line[604] == r"title: Moz$\mathbb  {Z}_{2^k}$arella: Efficient Vector-OLE and Zero-Knowledge Proofs Over $\mathbb  {Z}_{2^k}$"
      assert line[605] == r"title: \(\texttt  {POLKA}\): Towards Leakage-Resistant Post-Quantum CCA-Secure Public Key Encryption"
      assert line[606] == r"title: $\texttt  {zk-creds}$: Flexible Anonymous Credentials from zkSNARKs and Existing Identity Infrastructure"
      assert line[607] == r"title: Efficient supersingularity testing over $\mathbb  {F}_p$ and CSIDH key validation"
      assert line[608] == r"title: Time-Space Lower Bounds for Finding Collisions in Merkle-Damgård Hash Functions"
      assert line[609] == r"title: Post-quantum hash functions using $\mathrm  {SL}_n(\mathbb  {F}_p)$"
      assert line[610] == r"title: Low-Delay 4, 5 and 6-Term Karatsuba Formulae in $\mathbb  {F}_2[x]$ Using Overlap-free Splitting"
      ...
      assert line[613] == r"title: An $\mathcal  {O}(n)$ Algorithm for Coefficient Grouping"
      ...
      assert line[617] == r"title: Grotto: Screaming fast $(2 + 1)$-PC for $\mathbb  {Z}_{2^{n}}$ via (2, 2)-DPFs"
      assert line[618] == r"title: Homomorphic Sortition – Single Secret Leader Election for PoS Blockchains"
      assert line[619] == r"title: Hashing to elliptic curves over highly $2$-adic fields $\mathbb  {F}_{\!q}$ with $O(\log (q))$ operations in $\mathbb  {F}_{\!q}$"
      assert line[620] == r"title: More Efficient Zero-Knowledge Protocols over $\mathbb  {Z}_{2^k}$ via Galois Rings"
      assert line[621] == r"title: Practical Improvement to Gaudry-Schost Algorithm on Subgroups of $\mathbb  {Z}^{*}_{p}$"
      ...
      assert line[625] == r"title: CNF Characterization of Sets over $\mathbb  {Z}_2^n$ and Its Applications in Cryptography"
      assert line[626] == r"title: Differential Fault Attack on Rasta and $\text  {FiLIP} _ {\text  {DSM}}$"
      ...
      assert line[631] == r"title: Machine-Checked Security for $\mathrm  {XMSS}$ as in RFC 8391 and $\mathrm  {SPHINCS}^{+}$"
      assert line[632] == r"title: Minimal $p$-ary codes from non-covering permutations"
      assert line[633] == r"title: Provable Lattice Reduction of $\mathbb  Z^n$ with Blocksize $n/2$"
      assert line[634] == r"title: Batch Bootstrapping II: \\{\normalsize  Bootstrapping in Polynomial Modulus Only Requires $\tilde  O(1)$ FHE Multiplications in Amortization}"
      assert line[635] == r"title: End-to-End Encrypted Zoom Meetings:\\ Proving Security and Strengthening Liveness"
      assert line[636] == r"title: End-to-End Secure Messaging with Traceability\\ {\em  Only} for Illegal Content"
      assert line[637] == r"title: \large  {Almost Tight Multi-User Security under Adaptive Corruptions \& Leakages in the Standard Model}"
      assert line[638] == r"title: Practical Pre-Constrained Cryptography \\ \Large  {(or: Balancing Privacy and Traceability in Encrypted Systems)}"
      assert line[639] == r"title: \textbf  {Fully Adaptive \\Decentralized Multi-Authority \textsf  {ABE}}"
...
def test28_test():
  ...
  for option in ['-pdflua', '-pdf']:
    with tempfile.TemporaryDirectory() as tmpdirpath:
      ...
      assert meta['subtitle'] == 'Just a LaTeX subtitle'
...
def test39_test():
  # A simple test to make sure it compiles with acmart and amsart.
  path = Path('test39')
  docclass = [('acmart','sigconf,authordraft,twocolumn'),
              ('acmart', 'twocolumn'),
              ('acmart', ''),
              ('acmart','format=acmsmall'),
              ('amsart','')]
  styles = ['',
            'maketitle=iacrj',
            'maketitle=iacrj,footnotesymbols',
...
def test42_country():
  for option in ['-pdflua', '-pdf']:
    with tempfile.TemporaryDirectory() as tmpdirpath:
      ...
      res = run_engine(option, path.iterdir(), tmpdirpath)
      assert res['proc'].returncode != 0
      assert r'! Package metacapture Error: A country is required for each affiliation.' in res['log']
  
def test43_latin():
  """This is a test only for pdflatex to make sure it doesn't handle other input encodings."""
  for option in ['-pdf']:
    with tempfile.TemporaryDirectory() as tmpdirpath:
      path = Path('test43')
      res = run_engine(option, path.iterdir(), tmpdirpath)
